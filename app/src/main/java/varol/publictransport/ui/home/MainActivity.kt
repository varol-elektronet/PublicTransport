...
//    rv_routes_MainAct.adapter = routesAdapter
  ...
  /**
   * List filtered routes by [SegmentTypest] filter
   */
   ...
   */
  private suspend fun showPolyLineOnMap(segment: Segment) {
    try {
  ...
  fun getMyLocation() {
    ...
    }else
    {
     val gpsSnackBar: Snackbar = Snackbar.make(cl_bottomSheetHolder_ActMain,R.string.enable_gps_snack_text,Snackbar.LENGTH_LONG)
      gpsSnackBar.setAction(R.string.enable_gps_snack_action_text, View.OnClickListener {
